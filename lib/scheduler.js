...
class Scheduler {
  ...
  _executeFunction(fName, fInput) {
    let options = "";
    if (fInput) {
      options = `--data ${JSON.stringify(fInput)}`;
    }
    if (this.serverless.processedInput) {
      const slsOption = this.serverless.processedInput.options;
      for (const key in slsOption) {
        options += ` --${key} ${slsOption[key]}`;
      }
    }
    const cmd = options ? `serverless invoke local --function ${fName} ${options}` :
      `serverless invoke local --function ${fName}`;
