  ...
  run(opts) {
    ...
    for (const i in this.funcConfigs) {
      ...
      for (const j in fConfig.events) {
        ...
        this.serverless.cli.log(
          `scheduler: scheduling ${fConfig.id}/${eventData.name} with ${eventData.cron}${
            options.runSchedulesOnInit === true ? " (+init)" : ""
          }`
        );
        const cronFunc = new CronJob(
          eventData.cron,
          () => {
            ...
            func(this._getEvent(eventData), this._getContext(fConfig), () => {});
          },
          null,
          true,
          "America/Los_Angeles",
          null,
          options.runSchedulesOnInit === true ? true : false
        );
        cronFunc.start();
      }
    }
