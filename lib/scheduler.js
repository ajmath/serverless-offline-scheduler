...
class Scheduler {
  ...
  run(opts) {
    ...
    for (const i in this.funcConfigs) {
      ...
      for (const j in fConfig.events) {
        ...
        schedule.scheduleJob(eventData.cron, () => this._executeFunction(fConfig, eventData.input));

        if (options.runSchedulesOnInit) {
          return this._executeFunction(fConfig.id, eventData.input);
        }
      }
    }
  ...
  async _executeFunction(fConfig, fInput) {
    const modulePath = path.join(__dirname, `../../../${fConfig.moduleName}`);
    return require(modulePath)[fConfig.handlerFunction](fInput, this._getContext(fConfig));
  }
