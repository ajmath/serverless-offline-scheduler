...
const utils = require("./utils");const execSync = require('child_process').execSync;
...
class Scheduler {
    ...
    for (const i in this.funcConfigs) {
      ...
      for (const j in fConfig.events) {
        ...
        schedule.scheduleJob(eventData.cron, () => {
          const func = this._executeFunction(fConfig.id);
          if (func === undefined) {
  ...
  _executeFunction(fName) {
    return execSync(`serverless invoke local --function ${fName}`, {cwd: './', stdio: 'inherit' });
  }
